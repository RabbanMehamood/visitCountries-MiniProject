...
class VisitCountries extends Component {
  ...
  onClickBtn = id => {
    ...
    const visitList = countryList.map(each => {
      if (each.id === id) {
    ...
    this.setState({countryList: visitList})
  }

  render() {
    const {countryList} = this.state

    const lengthCountry = countryList.filter(each => each.isVisited === false)
      ...
      <div className="mainOuterContainer">
        <div className="countryContainer">
          ...
          <div className="flagsContainer">
            {lengthCountry.length === initialCountriesList.length ? (
              <div className="noVisitContainer">
                <p className="noVisit">No Countries Visited Yet</p>
              </div>
            ) : (
